...
import reference from "./supply-document-definition-contract.json";

/** DE-1A adds separate document registry tables without changing existing strict structures.
 ...
 */
export class SupplyDocumentPrerequisitesUnavailable extends Error {
  readonly status = 503;
  readonly code = "supply_document_prerequisites_unavailable";
  constructor(readonly diagnostic: string) {
    super("문서 등록에 필요한 자료구조를 확인할 수 없습니다.");
    this.name = "SupplyDocumentPrerequisitesUnavailable";
  }
}
...
const guardCatalog = `SELECT pg_catalog.jsonb_build_object(
 ...
 WHERE n.nspname=$1 AND p.proname='finance_assert_document_definitions'`;

export async function assertSupplyDocumentPrerequisites(db: Pick<PgDatabase, "exec">, schema = "public") {
  try {
    await assertSupplyReviewPrerequisites(db, schema);
    const rows = (await db.exec(guardCatalog, [schema]))[0]?.values ?? [];
    if (rows.length !== 1 || digest(rows[0][0]) !== expectedGuardDigest) throw new SupplyDocumentPrerequisitesUnavailable("definition_verifier_mismatch");
    const proof = (await db.exec(`SELECT "${schema}".finance_assert_document_definitions($1) AS proof`, [schema]))[0]?.values[0]?.[0] as Record<string, unknown> | undefined;
    if (!proof || proof.version !== reference.contract.version || proof.contractHash !== reference.contractHash || proof.schema !== schema
      ...
      || proof.structureCount !== reference.contract.strictRelations.length) throw new SupplyDocumentPrerequisitesUnavailable("definition_proof_mismatch");
    ...
    if (error instanceof SupplyDocumentPrerequisitesUnavailable) throw error;
    const code = (error as { code?: string })?.code;
    if (code === "40001" || code === "40P01") throw error;
    throw new SupplyDocumentPrerequisitesUnavailable(code === "55000" ? "dependency_definition_mismatch" : "dependency_unavailable");
  }
}
