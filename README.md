...
- 파생 규칙(`frontend/lib/filings/store.ts` `syncFilings`, 목록·요약 API 호출 시 재계산, 멱등):
  ...
  - IEPS 대행 실적: 용역분류 통합허가 계열 계약. 체결 = 계약일. 변경 = 체결 보고를 제출(또는 제외)한 **뒤에** 생긴 `contract_change_events`
    (그 전의 변경계약은 체결 보고에 최신 정보로 담는다). 이행 = 마지막 지급 단계 세금계산서 발행일(완료일(발행일)); 준공일자 값은 허가일(없으면 발행일).
    대행업무 기간: 시작일 = 계약일, 종료일 = 용역 종료일(없으면 최초허가는 +2년, 그 외는 상반기 시작 → 12/31, 하반기 시작 → +8개월). (변경)계약기간도 동일.
...
- 패널 **[사업장 검색]**(대행 실적보고): 본문의 사업장 검색 팝업(bplcCodeNmPopup, 검색어 `#file`)을 열어 법인 표기를 뗀 첫 낱말로 검색하고
  사업장명이 일치하는 결과 행을 클릭한다. 검색 버튼·결과 행은 실측 전이라 텍스트로 찾으며, 못 찾으면 팝업을 열어 둔 채 사람이 고른다.
  [폼 덤프]는 입력칸 외에 버튼·링크·표 헤더/첫 행도 기록한다(자동 클릭 대상 실측용).
- 패널 **[직인 첨부]**·**[서류 첨부]**(대행 실적보고): 직인은 `frontend/public/letter/stamp.png`(또는 `FILINGS_SEAL_PATH`), 서류는 MCM 계약 첨부를
  내려받아(체결=계약서, 변경=변경계약서, 이행=세금계산서) 각 칸의 [찾아보기] 파일 선택기를 가로채 넣고 [저장]을 누른다.
  이행 보고의 발주자 발급 이행증명서는 MCM 에 없어 직접 첨부한다. 사이트가 요구하는 '신청서 저장 후' 순서는 사람이 지킨다.
