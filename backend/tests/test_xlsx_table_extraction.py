...
class TestXLSXExtractorTableMethods:
    """XLSXExtractor 표 관련 메서드 테스트"""
    
    @pytest.fixture
    def extractor(self):
        """테스트용 XLSXExtractor 인스턴스"""
    ...
    def test_clean_cell_numeric(self, extractor):
        """숫자 셀"""
        assert extractor._clean_cell(12345) == "12345"
        assert extractor._clean_cell(123.45) == "123.45"
    ...
    def test_expand_merged_cells_complex(self, extractor):
        ...
        cells = [
            [
                {"text": "제목", "col_span": 3, "row_span": 1}
            ],
            [
                {"text": "항목", "col_span": 1, "row_span": 2},
                {"text": "분기", "col_span": 2, "row_span": 1}
            ],
            [
                {"text": "1분기", "col_span": 1, "row_span": 1},
                {"text": "2분기", "col_span": 1, "row_span": 1}
            ]
        ]
        result = extractor._expand_merged_cells(cells)
        
        assert len(result) == 3
        assert result[0] == ["제목", "", ""]  # 3열 병합
        assert result[1] == ["항목", "분기", ""]  # 항목은 2행 병합 시작, 분기는 2열 병합
        assert result[2] == ["", "1분기", "2분기"]  # 항목의 행 병합 영역
    
    # ==================== _merge_text_and_tables 테스트 ====================
    
    def test_merge_text_and_tables_no_tables(self, extractor):
        """표가 없는 경우"""
        result = extractor._merge_text_and_tables("Sheet1", [])
        assert result == ""
    ...
    def test_merge_text_and_tables_with_one_table(self, extractor):
        ...
        result = extractor._merge_text_and_tables("Sheet1", tables)
...
class TestXLSXSettings:
    ...
    def test_default_settings(self):
        ...
        assert config.xlsx.extract_tables is True
        assert config.xlsx.table_output_format == "markdown"
        assert config.xlsx.min_table_rows == 2
        assert config.xlsx.min_table_cols == 2
        assert config.xlsx.max_rows == 10000
        assert config.xlsx.max_cols == 100
    
    def test_extractor_uses_config(self):
        """Extractor가 설정을 사용하는지 확인"""
        config = ExtractionConfig()
        config.xlsx.max_rows = 500
        config.xlsx.max_cols = 50
        
        extractor = XLSXExtractor(config)
        
        assert extractor.max_rows == 500
        assert extractor.max_cols == 50
