...
import { BarChart3, PieChart, Timer, TrendingUp, Wallet } from "lucide-react";
import ApexChart from "@/components/contracts/dashboard/ApexChart";
import type { ContractCollectionRow, ContractCollectionsStatus } from "@/lib/ieps/collections-types";
...
/**
 * 수금 현황 — 용역/연도 필터 + 수금 리스트(계약 상세 딥링크) +
 * 연도별 수금액 추이 / 용역별 수금 비율 / 이중 도넛 / 3개년 평균 수금 소요기간.
 * 연도 분류는 모두 해당 용역의 계약일을 기준으로 한다.
 */
export function CollectionsSection({ theme }: { theme: CdTheme }) {
  const router = useRouter();
  const pal = chartPalette(theme);
  const [data, setData] = useState<ContractCollectionsStatus | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/contracts/billing/collections", { cache: "no-store" });
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error((body as { error?: string })?.error ?? "HTTP " + res.status);
        }
        const json = (await res.json()) as ContractCollectionsStatus;
        if (cancelled) return;
        setData(json);
  ...
  const trendSeries = useMemo(() => {
    const totalCollected = trendYears.map((y) => Math.round(collectedByYear.get(y)?.total ?? 0));
    const totalRatio = trendYears.map((y) => {
      const orders = ordersByYear.get(y)?.total ?? 0;
      const collected = collectedByYear.get(y)?.total ?? 0;
      return orders > 0 ? Number(((collected / orders) * 100).toFixed(1)) : 0;
    });
    const series: Array<{ name: string; type: "column" | "line"; data: number[] }> = [
      { name: "전체 수금액", type: "column", data: totalCollected },
    ];
    if (category) {
      series.push({
        name: `${category} 수금액`,
        type: "column",
        data: trendYears.map((y) => Math.round(collectedByYear.get(y)?.byCategory[category] ?? 0)),
      });
    }
    series.push({ name: "전체 수금 비율", type: "line", data: totalRatio });
    if (category) {
      series.push({
        name: `${category} 수금 비율`,
        type: "line",
        data: trendYears.map((y) => {
          ...
          const collected = collectedByYear.get(y)?.byCategory[category] ?? 0;
          return orders > 0 ? Number(((collected / orders) * 100).toFixed(1)) : 0;
        }),
      });
    }
    return series;
  }, [trendYears, collectedByYear, ordersByYear, category]);
  ...
  /* ---------------- (b) 해당연도 용역별 수금 비율 (가로 막대) ---------------- */
  const ratioByCategory = useMemo(() => {
    const yearCollected = isAllYears ? aggregateAll(collectedByYear) : collectedByYear.get(year);
    ...
    return DASHBOARD_CATEGORIES.map((cat) => {
      ...
      const collected = yearCollected?.byCategory[cat] ?? 0;
      return {
        category: cat,
        ratio: orders > 0 ? Number(((collected / orders) * 100).toFixed(1)) : 0,
        collected,
      };
    }).filter((e) => e.ratio > 0 || e.collected > 0);
  }, [collectedByYear, ordersByYear, year, isAllYears]);
  ...
  const yearEntry = useMemo(
    () => (isAllYears ? aggregateAll(collectedByYear) : collectedByYear.get(year)),
    [collectedByYear, year, isAllYears]
  );
  ...
  /* ---------------- (d) 3개년 평균 수금 소요기간 (발행일 → 수금일) ---------------- */
  const periodYears = useMemo(() => {
    const base = /^\d{4}$/.test(year) ? Number(year) : new Date().getFullYear();
    return [String(base - 2), String(base - 1), String(base)];
  }, [year]);

  const periodStats = useMemo(() => {
    // 계약일 연도별로 (수금일 - 발행일) 평균 소요일수를 전체/선택 용역으로 집계
    ...
    for (const row of allRows) {
      ...
      if (!entry) continue;
      if (!row.collectedAt || !row.invoiceIssuedAt) continue;
      const issued = Date.parse(row.invoiceIssuedAt);
      const collected = Date.parse(row.collectedAt);
      if (!Number.isFinite(issued) || !Number.isFinite(collected)) continue;
      const days = Math.max(0, Math.round((collected - issued) / (1000 * 60 * 60 * 24)));
      entry.allSum += days;
      entry.allCnt += 1;
    ...
    return periodYears.map((y) => {
      ...
      return {
        year: y,
        all: e.allCnt > 0 ? Math.round(e.allSum / e.allCnt) : null,
        cat: category && e.catCnt > 0 ? Math.round(e.catSum / e.catCnt) : null,
      };
    });
  ...
  const periodOptions = useMemo<ApexOptions>(
    () => ({
      ...
      dataLabels: { enabled: false },
      grid: { borderColor: pal.grid, strokeDashArray: 3, padding: { left: 8, right: 12 } },
      legend: {
        show: true,
        position: "top",
        horizontalAlign: "left",
      ...
      xaxis: {
        categories: periodYears.map((y) => `${y}년`),
        labels: { style: { colors: pal.muted, fontSize: "10px", fontWeight: 600 } },
        axisBorder: { show: false },
        axisTicks: { show: false },
      },
      ...
      tooltip: {
        theme: theme === "dark" ? "dark" : "light",
        y: { formatter: (v: number | null) => (v == null ? "-" : `${v}일`) },
      },
    }),
  ...
  const handleExport = async (format: "xlsx" | "pdf") => {
    ...
    try {
      ...
      const res = await fetch("/api/contracts/billing/collections/export?" + params.toString());
      ...
      a.download = filenameFromDisposition(res, `수금리스트_${stamp}.${format}`);
  ...
  const openContract = (row: ContractCollectionRow) => {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.8fr)_minmax(0,0.62fr)] gap-4">
        ...
        <div className="cdb-box p-4 min-w-0">
          ...
          <p className="text-[10px] font-bold mb-4" style={{ color: "var(--cd-faint)" }}>
            계약일 기준 · 막대 = 수금액, 선 = 수주액 대비 수금 비율
          </p>
          <ApexChart
            key={`col-trend-${theme}-${category ?? "all"}`}
        ...
        <div className="cdb-box p-4 min-w-0">
          ...
          {ratioByCategory.length === 0 ? (
            <EmptyHint label="표시할 수금 데이터가 없습니다." height={330} />
          ) : (
            <ApexChart
              key={`col-ratiobar-${theme}-${year}`}
              options={ratioBarOptions}
              series={[{ name: "수금 비율", data: ratioByCategory.map((e) => e.ratio) }]}
              type="bar"
              height={330}
            />
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="min-w-0">
              ...
              {shareDonut.series.reduce((a, b) => a + b, 0) <= 0 ? (
                ...
                <ApexChart
                  key={`col-share-${theme}-${year}-${category ?? "all"}`}
                  options={donutBase(shareDonut.labels, shareDonut.colors, "전체 수금액", yearEntry?.total ?? 0)}
            ...
            <div className="min-w-0">
              ...
              {!category ? (
                ...
                <ApexChart
                  key={`col-subtype-${theme}-${year}-${category}`}
                  options={donutBase(
                    subtypeDonut.labels,
        ...
        <div className="cdb-box p-4 min-w-0">
          ...
          <p className="text-[10px] font-bold mb-4" style={{ color: "var(--cd-faint)" }}>
            계산서 발행일 → 수금일까지의 평균 기간
          </p>
          {periodStats.every((e) => e.all == null) ? (
            <EmptyHint label="표시할 수금 소요기간 데이터가 없습니다." height={300} />
          ) : (
            <ApexChart
              key={`col-period-${theme}-${year}-${category ?? "all"}`}
