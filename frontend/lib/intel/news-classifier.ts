...
function buildPrompt(title: string, description: string): string {
  return (
    ...
    "- isSignal=false 예: 해외공장, 실적/주가/목표가 분석, 단순 시황, 지분·M&A, 정책/행정 일반, 이미 가동 중 단순 소개.\n" +
    ...
    '"location": "위치 또는 null", "scale": "규모표현 또는 null", "confidence": "high|medium|low", "summary": "한국어 한 줄 요약"}'
  );
}
...
function normalize(j: Record<string, unknown>): NewsClassification {
 ...
 */
export async function classifyNews(title: string, description: string): Promise<NewsClassification | null> {
