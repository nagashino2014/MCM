...
import { INTEL_SIGNAL_TYPE_LABELS } from "@/lib/intel/signal-extractor";
...
export function IntelBoard() {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/sales/intel/signals", { cache: "no-store" });
  ...
  const collect = async () => {
    ...
    setCollectMsg(null);
    try {
      const res = await fetch("/api/sales/intel/collect", { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error ?? `HTTP ${res.status}`);
      setCollectMsg(`수집 완료 · 조회 ${data.scanned} / 후보 ${data.candidates} / 신규 ${data.inserted} / 매칭 ${data.matched}`);
      await reload();
    } catch (e) {
      setCollectMsg(`수집 실패: ${(e as Error).message}`);
    } finally {
      setCollecting(false);
    }
  };
  ...
  const hasFilter = !!(q || signalType || matchStatus || status);
    ...
    <div className="cdash cd-fields-white p-2" data-theme={theme}>
      <CdPageHeader
        ...
        actions={
          <>
            {canEdit && (
              <button className="cd-btn cd-btn-primary cd-btn-sm" onClick={collect} disabled={collecting}>
                <RefreshCw className={`w-4 h-4 ${collecting ? "animate-spin" : ""}`} /> {collecting ? "수집 중…" : "지금 수집"}
              </button>
            )}
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        ...
        {hasFilter && (
          <button className="cd-btn cd-btn-ghost cd-btn-sm" onClick={() => { setQ(""); setSignalType(""); setMatchStatus(""); setStatus(""); }}>
