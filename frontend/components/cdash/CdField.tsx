...
function FieldShell({ label, required, hint, error, className, children, labelExtra, descriptionId, controlId }: FieldShellProps) {
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      {label != null && (
        <div className="flex items-center justify-between gap-2">
          <label htmlFor={controlId} className="cd-label">
            {label}
            {required && <span aria-hidden="true" className="cd-error-text ml-0.5">*</span>}
          </label>
          {labelExtra}
        </div>
      )}
      {children}
      {error ? (
        <span id={descriptionId} className="text-xs cd-error-text">{error}</span>
      ) : hint ? (
        <span id={descriptionId} className="text-xs cd-text-faint">{hint}</span>
      ) : null}
    </div>
  );
}
...
export const CdInput = forwardRef<HTMLInputElement, CdInputProps>(function CdInput(
  ...
  const controlId = rest.id ?? `${descriptionId}-control`;
  const describedBy = [rest["aria-describedby"], (hint || error) && descriptionId].filter(Boolean).join(" ") || undefined;
  return (
    <FieldShell label={label} required={required} hint={hint} error={error} labelExtra={labelExtra} className={className} descriptionId={descriptionId} controlId={controlId}>
      <input ref={ref} className={cn("cd-input", error && "border-[color:var(--cd-error)]")} {...rest} id={controlId} aria-required={required} aria-invalid={error ? true : rest["aria-invalid"]} aria-describedby={describedBy} />
    </FieldShell>
  );
});
...
export const CdSelect = forwardRef<HTMLSelectElement, CdSelectProps>(function CdSelect(
  ...
  return (
    <FieldShell label={label} required={required} hint={hint} error={error} labelExtra={labelExtra} className={className} descriptionId={descriptionId} controlId={controlId}>
      <select ref={ref} className={cn("cd-select", error && "border-[color:var(--cd-error)]")} {...rest} id={controlId} aria-required={required} aria-invalid={error ? true : rest["aria-invalid"]} aria-describedby={describedBy}>
        {children}
      </select>
    </FieldShell>
  );
...
export const CdTextarea = forwardRef<HTMLTextAreaElement, CdTextareaProps>(function CdTextarea(
  ...
  return (
    <FieldShell label={label} required={required} hint={hint} error={error} labelExtra={labelExtra} className={className} descriptionId={descriptionId} controlId={controlId}>
      <textarea ref={ref} className={cn("cd-textarea", error && "border-[color:var(--cd-error)]")} {...rest} id={controlId} aria-required={required} aria-invalid={error ? true : rest["aria-invalid"]} aria-describedby={describedBy} />
    </FieldShell>
  );
});
...
export const CdDateInput = forwardRef<HTMLInputElement, CdDateInputProps>(function CdDateInput(
  ...
  const descriptionId = useId();
  const controlId = rest.id ?? `${descriptionId}-control`;
  const describedBy = [rest["aria-describedby"], (hint || error || malformed) && descriptionId].filter(Boolean).join(" ") || undefined;
  ...
  return (
    ...
    >
      <input
        ...
        aria-invalid={error || malformed ? true : rest["aria-invalid"]}
