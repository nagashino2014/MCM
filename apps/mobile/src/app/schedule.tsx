    ...
    <SafeAreaView edges={['bottom']} className="flex-1 bg-cd-bg">
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text className="mt-10 text-center text-cd-faint">이 달 일정이 없습니다</Text>
          }
  ...
  return (
    <View className="rounded-2xl border border-cd-border bg-cd-card p-4">
      <View className="flex-row items-center gap-2">
        ...
        {a.activityType ? <Text className="text-xs text-cd-muted">{a.activityType}</Text> : null}
      </View>
      <Text className="mt-2 text-base font-bold text-cd-text">
        {a.projectTitle || a.facilityName || '(제목 없음)'}
      </Text>
      {a.facilityName && a.projectTitle ? (
        <Text className="text-xs text-cd-muted">{a.facilityName}</Text>
      ) : null}
      {a.summary ? (
        <Text className="mt-1 text-sm text-cd-muted">{a.summary}</Text>
      ) : null}
      {a.contacts?.length ? (
          ...
          {a.contacts.map((c, i) => {
            ...
            return (
              <View key={c.id ?? i} className="flex-row items-center justify-between">
                <Text className="text-sm text-cd-text">{c.personName}</Text>
                {phone ? (
