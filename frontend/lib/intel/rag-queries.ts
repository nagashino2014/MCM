...
function buildBriefingPrompt(question: string, hits: RagSearchHit[]): string {
  ...
  return (
    ...
    '      "desc": "위치·규모·금액·진행단계·사업기간을 간결한 2~3문장으로. 사업기간(착수~완공·준공 시점)은 신호에 있으면 반드시 포함하고, 없으면 \'사업기간 미공개\'라고 명시(영업 적기 판단에 필수). 각 문장은 마침표로 끝내고(화면에서 문장 단위 줄바꿈됨) 신호번호 인용 [3]",\n' +
    '      "facts": [{"label": "사업기간", "value": "2028~2030"}],  // 확인된 수치만 최대 3개 — 사업기간이 확인되면 반드시 첫 항목으로, 그 외 규모/금액/완공목표. 없으면 빈 배열\n' +
