...
const SYSTEM_SOURCES_DOC = `system.contracts: 계약 마스터. measures={amount(계약금액),count}, dims=[ref.contract,dim.category(용역분류)], times={contract_date}
system.participants: 수행인력. measures={contract_id(count_distinct 로 참여 계약 수),duration_days(수행일수)}, dims=[ref.employee,ref.contract], times={participated_from}
...
규칙: 비용·금액 요청은 대개 approval_docs 태그 집계. 마진율·비율은 composite. 인원별 지표(ref.employee)는 민감 정보다. 존재하지 않는 태그·필드·지표 키를 만들어내지 말 것.`,
      user: prompt,
      maxTokens: 800,
    });
