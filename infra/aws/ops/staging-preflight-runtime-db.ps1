...
$config = if ($Component -eq "next") {
  @{ Container = "next"; Role = "mcm_app"; Secret = "mcm-ieps-staging/db-app"; ApplicationSecret = "mcm-ieps-staging/app"; ApplicationKeys = @("AUTH_SECRET", "ADMIN_USERNAME", "ADMIN_PASSWORD", "SOLAPI_API_KEY", "SOLAPI_API_SECRET", "ANTHROPIC_API_KEY"); TaskRole = "mcm-ieps-staging-ecs-task"; ExecutionRole = "mcm-ieps-staging-ecs-execution-next"; Family = "mcm-ieps-staging-next" }
} else {
  @{ Container = "worker"; Role = "mcm_worker"; Secret = "mcm-ieps-staging/db-worker"; ApplicationSecret = "mcm-ieps-staging/app"; ApplicationKeys = @("DART_API_KEY", "DATA_GO_KR_API_KEY"); TaskRole = "mcm-ieps-staging-ecs-task-worker"; ExecutionRole = "mcm-ieps-staging-ecs-execution-worker"; Family = "mcm-ieps-staging-worker" }
}
