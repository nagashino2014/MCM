...
const COMPLETION_STATEMENT: DeliverableSpec = {
  ...
  blocks: [
    ...
    { kind: "spacer", heightPt: 22 },
    {
      ...
      numbering: "decimal",
      rowGapPt: 10,
      // 라벨 폭 통일(2026-08-19) — 최장 '착수년월일'에 맞춰 양끝을 벌린다
      rows: [
        { label: "용  역  명", binding: "contract.title", format: "text" },
        { label: "용 역 금 액", binding: "contract.amount", format: "amountBoth" },
        { label: "준 공 금 액", binding: "completion.cumulativeAmount", format: "amountBoth" },
...
const SERVICE_RESULT_REPORT: DeliverableSpec = {
  ...
  blocks: [
    ...
    { kind: "spacer", heightPt: 22 },
    {
      ...
      numbering: "decimal",
      rowGapPt: 10,
      // 라벨 폭 통일(2026-08-19 사용자 확정) — 최장 '계약상대자'에 맞춰 양끝을 벌린다
      rows: [
        { label: "발 주 번 호", binding: "contract.orderNo", format: "text" },
        { label: "용  역  명", binding: "contract.title", format: "text" },
        { label: "계약상대자", binding: "company.name", format: "text" },
        { label: "계 약 기 간", binding: "contract.period", format: "period" },
        { label: "착  수  일", binding: "contract.startDate", format: "dateKorean" },
        { label: "준  공  일", binding: "completion.actualDate", format: "dateKorean" },
        { label: "첨 부 자 료", binding: "completion.attachList", format: "multiline" },
      ],
    },
    { kind: "spacer", heightPt: 20 },
    {
