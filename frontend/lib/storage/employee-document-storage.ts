...
export interface EmployeeDocumentObject {
...
export function buildEmployeeDocumentStorageKey(params: {
  ...
  const safeName = sanitizeFilename(`${base || "증빙서류"}${ext}`);
  const storageKey = [
    "employees",
    sanitizePathSegment(params.employeeName || params.employeeId),
    sanitizePathSegment(params.documentType || "document"),
    `${Date.now()}-${safeName}`,
  ].join("/");
  return { storageKey, fileName: safeName };
}

export async function putEmployeeDocument(
  storageKey: string,
  body: Buffer,
  contentType: string
  ...
  if (localRoot) {
    ...
    return {
      ...
      storageBucket: null,
      storageKey,
      publicPath: "/api/admin/employee-documents/download?key=" + encodeURIComponent(storageKey),
    };
  }
...
export async function getEmployeeDocument(storageKey: string): Promise<EmployeeDocumentObject> {
  if (storageKey.includes("..") || storageKey.startsWith("/") || storageKey.startsWith("\\")) {
    throw new Error("잘못된 문서 경로입니다.");
  }
  const bucket = process.env.MCM_STORAGE_BUCKET?.trim();
  if (!bucket) {
    throw new Error("MCM_STORAGE_BUCKET 환경변수가 필요합니다.");
  }
