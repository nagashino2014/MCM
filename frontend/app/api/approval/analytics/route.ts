...
async function resolveAccess(): Promise<{ userId: string; manager: boolean }> {
  try {
    const actor = await requirePermission("approval.manage", { fallbackRoles: ["admin"] });
    return { userId: actor.userId, manager: true };
  } catch {
    const actor = await requirePermission("approval.view");
    return { userId: actor.userId, manager: false };
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ metrics, manager: access.manager });
  } catch (err) {
