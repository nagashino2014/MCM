...
.DESCRIPTION
  staging-deploy-next.ps1 과 동일한 절차의 converter 판이다. 태스크 정의가 고정 태그를
  가리키므로 force-new-deployment 만으로는 새 이미지가 반영되지 않는다.
  1) 빌드 → 설명 태그 + latest 로 푸시
  2) 현재 태스크 정의를 받아 image 만 새 태그로 바꿔 새 리비전 등록
  3) update-service 로 새 리비전 적용

  - 빌드 컨텍스트는 repo 루트, Dockerfile 은 converter/Dockerfile, 플랫폼 linux/amd64.
  - 이미지 베이스가 ubuntu:24.04(LibreOffice 24.2)라 첫 빌드는 내려받기가 오래 걸린다.
  - desired-count 1 고정(서비스 디스커버리 converter.local 로 next 가 호출).

.EXAMPLE
  .\infra\aws\ops\staging-deploy-converter.ps1
  .\infra\aws\ops\staging-deploy-converter.ps1 -Tag hwpfonts-20260901-1200
  .\infra\aws\ops\staging-deploy-converter.ps1 -Wait        # 배포 안정화까지 대기
  .\infra\aws\ops\staging-deploy-converter.ps1 -SkipBuild -Tag <이미푸시한태그>
#>
param(
...
$service   = "mcm-ieps-staging-converter"
$repo      = "mcm-ieps-staging-converter"
$container = "converter"
...
if (-not $SkipBuild) {
  ...
  docker build --platform linux/amd64 -f (Join-Path $repoRoot "converter\Dockerfile") -t $image -t "${registry}/${repo}:latest" $repoRoot
...
$tdPath = Join-Path $env:TEMP "mcm_converter_taskdef.json"
...
Log "완료. 태그 $Tag 배포. 새 태스크 기동까지 1~2분."
Log "확인: 전자결재 증명신청서 승인 → 개인문서함 발급본(명조·직인 위치) 또는 발급 관리 재생성"
