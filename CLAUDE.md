...
- **재무 통합 임시 배포 중지(2026-09-22, 유지):** 스테이징에 재무 SQL 28개를 적용했고 정의 검사 7/7을 확인한 뒤 Next 624를 배포했다(ADT·intel 배치는 618에 고정). 인증된 급여·카드·부가세·전표 화면 검증은 아직 끝나지 않았다. 사용자 요청에 따라 추가 Next 배포를 계속 중지한다. 재무 SQL 226~264를 일괄 재적용하지 말고, 개별 재실행 전에도 각 파일의 설치 계약을 확인한다. 이 중지는 사용자 승인 후 별도 커밋으로 해제한다.
