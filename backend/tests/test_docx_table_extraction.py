...
class TestDOCXExtractorTableMethods:
    """DOCXExtractor 표 관련 메서드 테스트"""
    
    @pytest.fixture
    def extractor(self):
        """테스트용 DOCXExtractor 인스턴스"""
    ...
    def test_table_to_markdown_3x3(self, extractor):
        ...
        assert "구분" in lines[0]
        assert "---" in lines[1]
        assert "데이터1" in lines[2]
    ...
    def test_expand_merged_cells_no_merge(self, extractor):
        ...
        cells = [
            [
                {"text": "A", "col_span": 1, "row_span": 1, "is_merge_continue": False},
                {"text": "B", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ],
            [
                {"text": "C", "col_span": 1, "row_span": 1, "is_merge_continue": False},
                {"text": "D", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ]
        ]
    ...
    def test_expand_merged_cells_col_span(self, extractor):
        ...
        cells = [
            [
                {"text": "헤더", "col_span": 2, "row_span": 1, "is_merge_continue": False}
            ],
            [
                {"text": "값1", "col_span": 1, "row_span": 1, "is_merge_continue": False},
                {"text": "값2", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ]
        ]
        result = extractor._expand_merged_cells(cells)
    ...
    def test_expand_merged_cells_row_span(self, extractor):
        ...
        cells = [
            [
                {"text": "A", "col_span": 1, "row_span": 2, "is_merge_continue": False},
                {"text": "B", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ],
            [
                {"text": "C", "col_span": 1, "row_span": 1, "is_merge_continue": False}
    ...
    def test_expand_merged_cells_with_merge_continue(self, extractor):
        """병합 연속 셀이 있는 표 (DOCX vMerge)"""
        cells = [
            [
                {"text": "A", "col_span": 1, "row_span": 1, "is_merge_continue": False},
                {"text": "B", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ...
            [
                {"text": "", "col_span": 1, "row_span": 1, "is_merge_continue": True},  # vMerge continue
                {"text": "D", "col_span": 1, "row_span": 1, "is_merge_continue": False}
            ]
        ]
        result = extractor._expand_merged_cells(cells)
        ...
        assert len(result) == 2
        assert result[0] == ["A", "B"]
        assert result[1] == ["", "D"]  # is_merge_continue=True는 빈 셀
    ...
    def test_merge_text_and_tables_empty_text(self, extractor):
        """빈 텍스트에 표 추가"""
        tables = [
            TableData(
                table_index=0,
        ...
        result = extractor._merge_text_and_tables("", tables)
        
        assert "<!-- 표 1 -->" in result
        assert "| A | B |" in result


class TestDOCXSettings:
    """DOCX 설정 테스트"""
    ...
    def test_default_settings(self):
        ...
        assert config.docx.extract_tables is True
        assert config.docx.table_output_format == "markdown"
        assert config.docx.min_table_rows == 2
        assert config.docx.min_table_cols == 2
    
    def test_extractor_uses_config(self):
        """Extractor가 설정을 사용하는지 확인"""
        config = ExtractionConfig()
        config.docx.extract_tables = False
        
        extractor = DOCXExtractor(config)
        
        assert extractor.config.docx.extract_tables is False
