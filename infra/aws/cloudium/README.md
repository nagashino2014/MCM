...
### 남은 확인 사항
1. 클라이언트 **로그아웃 → 재로그인** 또는 재부팅 (정책 동기화)
2. **설정 항목** — "PC매체 제어"는 매체 반출 통제일 가능성이 높다. 드라이브 접근 허용은
   "프로그램 제어 / 허용 애플리케이션" 등 별도 항목일 수 있다
3. 등록 형식 — 전체 경로(`C:\Windows\System32\Robocopy.exe`) 필요 여부, 32비트 버전 병행 등록
4. **탐색기로 T: → D: 폴더 복사가 되는지** (반출 자체가 통제될 수도 있다)

위 1~3 이 풀리면 `04-pilot-measure.ps1` 을 그대로 쓸 수 있다. 안 풀리면
[vendor-export-request.md](vendor-export-request.md) 로 벤더에 문의할 것(문의용 정리 완료).
