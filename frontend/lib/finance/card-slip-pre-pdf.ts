// 법인카드 전자 전표(매출전표) PDF — 바로빌 매입내역 1건 = 전표 1장. (사전 생성 파이프라인 전용)
// ⚠2026-08-28 브랜치 통합: 같은 목적의 렌더러가 두 계열로 나뉘어 있었다 —
//   이 파일(구 card-slip-pdf.ts)은 사전 생성 배치(card-slip.ts + card-slip-tick) 전용으로 이름을 바꿔 보존했고,
//   card-slip-pdf.ts 는 지출결의 첨부 자동 생성(card-slips/route, next:514 계열)이 쓴다. 정리는 후속 판단.
