...
- 한 파일에 여러 건이 들어 있는 묶음 전표는 **건별로 쪼개** 기록한다. 경계는
  **① 페이지 단위**(주문번호가 정확히 1개인 페이지 = 전표 하나; 쿠팡 뷰어가 이 모양) →
  ② 건마다 반복되는 머리글(`receiptKeywords`, 건수가 맞을 때만) → ③ 주문번호 위치 순으로 잡는다.
  ⚠ ③은 쿠팡처럼 거래일시·승인번호가 주문번호보다 **앞에** 찍히는 양식에서 옆 전표의
  날짜·승인번호를 끌어오는 사고가 있었다(2026-08). 그래서 페이지 단위가 1순위다.
...
- 원장 기준 커버리지: 가맹점명이 쇼핑몰/PG(`SHOP_STORE_KEYWORDS`)인데 전표가 안 붙은 결제 목록 —
  수집 누락을 여기서 잡는다.
- **매칭 제외**(198): 같은 쇼핑몰 계정으로 산 개인카드 결제는 원장에 상대가 없어 영원히
  미매칭으로 남는다 → [제외] 로 걷어낸다(합계·자동 매칭 대상에서 빠짐). 제외하면 기존 연결도
  풀리고, 잘못 눌렀으면 [복원] 후 다시 매칭하면 된다. 재업로드해도 제외 표시는 유지된다.
