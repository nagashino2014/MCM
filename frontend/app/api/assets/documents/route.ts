...
// GET ?key= : 차량 서류(계약서/등록증) PDF 인라인 스트리밍 — PDF 뷰어 팝업의 iframe 소스.
// 계약서 문서 조회 라우트(/api/contracts/documents)와 동일 패턴(same-origin 인증 프록시,
// Content-Disposition 없음 = 인라인 렌더). 키는 assets/ 프리픽스만 허용.
export async function GET(req: NextRequest) {
  try {
    await requireSession();
    const key = req.nextUrl.searchParams.get("key")?.trim();
    if (!key || key.includes("..") || !key.startsWith("assets/")) {
