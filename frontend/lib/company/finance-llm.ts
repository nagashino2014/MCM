...
async function callLlm(block: Record<string, unknown>, prompt: string, maxTokens: number): Promise<Record<string, unknown> | null> {
...
const FINANCE_PROMPT =
  "이 파일이 한국 국세청 홈택스에서 출력한 표준재무제표(표준재무상태표·표준손익계산서 등 '표준' 서식)인지 먼저 판별하세요. " +
  "일반 재무제표·감사보고서·결산서 등 표준재무제표가 아니면 isStandard=false 로만 답하세요.\n" +
  "표준재무제표가 맞으면 문서에 포함된 회계연도별(당기/전기 등) 값을 추출해 JSON만 출력하세요(설명·코드블록 금지).\n" +
  "매우 중요: 문서에 인쇄된 숫자만 사용하고, 금액은 콤마·단위 없이 원 단위 정수 문자열로 쓰세요(문서가 천원 단위면 1000을 곱해 원 단위로 환산).\n" +
...
/** 홈택스 표준재무제표 파싱. 키 미설정이면 null. 표준재무제표가 아니면 빈 rows + isStandard=false. */
export async function parseFinancialStatementWithLlm(
  file: File
): Promise<{ isStandard: boolean; rows: FinanceParsedRow[] } | null> {
  const raw = await callLlm(await toContentBlock(file), FINANCE_PROMPT, 2000);
  if (!raw) return null;
