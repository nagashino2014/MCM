...
- **한 주문에 상품이 여러 개일 때** `ordPrdSeq`(주문 내 상품 순번)를 어떻게 줘야 하는지 확인되지 않아 1 로 고정해 뒀다.
  상품별로 전표가 나뉘는 구조라면 순번을 순회해야 한다. 판별은 `receipt` 명령으로 한다:

  ```bash
  npm run receipts -- receipt --ordNo <상품이 여러 개인 주문번호> --seq 1,2,3
  ```

  순번별 전표를 열어 무엇이 달라지는지 토큰 단위로 보여준다. 품목명이 통째로 바뀌면 상품별 전표이고,
  순번 값·발행 시각만 다르면 주문 단위 문서다.
