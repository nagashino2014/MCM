...
import type {
  ContractUnbilledOrdersYearly,
  ContractUnbilledRow,
  ContractUnbilledStatus,
} from "./unbilled-types";

// 클라이언트 공용 타입은 unbilled-types.ts 에 정의하고 여기서 재노출한다.
export * from "./unbilled-types";
...
/**
 * 수주/수금/발행 현황 — 미발행 현황.
 * 세금계산서 미발행(invoice_issued=0) 청구·수금 단계 전체와,
...
export async function getContractUnbilledStatus(): Promise<ContractUnbilledStatus> {
  ...
  // 미발행 비율(미발행액 / 수주액) 계산용 — 연도/분류별 수주액
  ...
  const mapped: ContractUnbilledRow[] = rows.map((r) => ({
