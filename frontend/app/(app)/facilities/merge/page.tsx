    ...
    return (
      <div className="p-8 flex items-center justify-center">
        <div className="glass-card rounded-3xl p-10 text-center max-w-md">
          ...
          <p className="text-sm text-stone-500">사업장 병합은 편집자 이상 권한이 필요합니다.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-2">
        ...
        <div className="relative z-10 flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-stone-600 text-base max-w-3xl">
              사업자번호·상호·사업장 소재지 일치 후보를 카테고리별로 확인하고,
              직접 상호 검색으로 병합 대상도 구성할 수 있습니다.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:h-[calc(100vh-260px)] lg:min-h-[520px]">
        <section className="glass-panel rounded-3xl p-6 reveal delay-1 flex flex-col gap-5 min-h-0 overflow-hidden">
          <div className="flex flex-col gap-3 min-h-0 flex-1">
            <h3 className="font-bold text-stone-700 mb-3">중복 후보 그룹 ({groups.length})</h3>
            {error && <div className="text-red-600 text-sm font-bold mb-3">조회 실패: {error}</div>}
            {loading && <div className="text-stone-400 text-sm py-10 text-center">로딩 중…</div>}
            {!loading && groups.length === 0 && !error && (
              <div className="text-stone-400 text-sm py-10 text-center flex flex-col items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[195px_1fr] gap-3 min-h-0 flex-1">
              <div className="bg-white/35 border border-white/60 rounded-2xl p-3 flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedMatchType === matchType
                        ...
                        : "bg-white/60 text-stone-700 border-white/70 hover:bg-white"
                    )}
                  >
              ...
              <div className="bg-white/35 border border-white/60 rounded-2xl p-3 min-h-0 overflow-y-auto scrollbar-hide">
          ...
          <div className="border-t border-stone-200/70 pt-5 shrink-0">
            <h3 className="font-bold text-stone-700 mb-2">직접 검색 병합</h3>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-xl min-w-[64px] px-3 py-2 text-xs font-bold text-white bg-primary hover:bg-primary/90 shadow-sm flex shrink-0 items-center justify-center gap-1 whitespace-nowrap disabled:opacity-60"
              >
            ...
            {manualSelected.length > 0 && (
              <div className="mt-2 flex items-center justify-between gap-2 text-xs text-stone-500">
                ...
                <button
                  ...
                  className="font-bold text-stone-500 hover:text-red-600 flex items-center gap-1"
                >
                ...
                return (
                  <label
                    ...
                    className={cn(
                      ...
                      checked ? "bg-primary/10 border-primary/30" : "bg-white/40 border-white/50"
                    )}
                  >
                    <input
                      type="checkbox"
        ...
        <section className="glass-panel rounded-3xl p-6 reveal delay-2 min-h-0 overflow-y-auto scrollbar-hide">
          ...
          {activeFacilities.length >= 2 && (
            <div className="flex flex-col gap-5">
              <div className="flex items-start justify-between gap-3">
                ...
                {active?.kind === "auto" && (
                  <button
                    ...
                    disabled={excluding}
                    className="rounded-xl px-3 py-2 text-xs font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 disabled:opacity-60"
                  >
                    {excluding ? "제외 중…" : "목록에서 제외"}
                  </button>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold text-stone-800 flex items-center gap-2 truncate">
                          {formatCompanyName(facility.companyName) ?? facility.companyName}
                          <span className="text-[9px] font-bold uppercase tracking-wide bg-stone-200 text-stone-700 px-1.5 py-0.5 rounded">
                            {facility.source}
                          </span>
                          {isTarget && (
                            <span className="text-[9px] font-bold uppercase tracking-wide bg-primary text-white px-1.5 py-0.5 rounded">
                              TARGET
                            </span>
                          )}
                        </div>
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-bold text-stone-600">병합 이유</span>
                <select
              ...
              <div>
                <h4 className="font-bold text-stone-700 text-sm">필드별 우선순위</h4>
                <p className="text-xs text-stone-500 mt-0.5">
              ...
              <div className="flex flex-col gap-2">
                {MERGEABLE_FIELDS.map((field) => (
                  <div
                    key={field.key}
                    className="flex items-center gap-3 bg-white/40 border border-white/50 rounded-xl p-3"
                  >
                    <span className="text-xs font-bold text-stone-600 w-32 shrink-0">
                      {field.label}
                    </span>
                    <select
