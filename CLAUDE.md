...
- **재무 통합 임시 배포 중지(2026-09-22):** 이 커밋의 Next는 새 재무 SQL 28개(다른 기능의 SQL 246·252~254·259·261·262 제외)에 의존한다. 스테이징에는 226~232만 적용됐고 233은 실패해 롤백됐다. 나머지는 `243 → 233~242 → 244~245 → 247 → 255~258 → 260 → 263~264` 순서로 **한 번의** `-AccountingMaintenance` 절차에서 적용한다. 233보다 먼저 243의 `card_tax_reviews`가 필요하다. 수정 후보의 전체 설치 리허설과 독립 검토를 마치고, 스테이징에서 모든 재무 SQL의 적용 성공을 확인하기 전에는 Next를 배포하지 않는다. SQL 적용이 실패하거나 복구 표식이 남으면 배포를 계속하지 않는다. 적용·배포 확인 뒤 이 임시 문구를 별도 커밋으로 제거한다.
