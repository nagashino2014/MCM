...
### 10.7 M6-A 홈 재구성 (2026-07-30, 커밋 8b827c0, OTA 06bd2ccb — 서버 변경 없음)

**전제: 데스크탑 홈이 Soft Glass Ink 개편으로 바뀌었다**(커밋 242c195). 실사 결과 구조는
① 스탯 4카드(결재 대기·안읽은 메일·오늘 일정·경과 미입력 + 델타 문구)
② 고정 영역(좌: 결재 대기·안읽은 메일 / 우: **월 캘린더**)
③ **사용자 배치 영역** — 위젯 10종을 편집 모드에서 표시·위치·크기 조정, `/api/home/layout` 에 서버 저장.
위젯 카탈로그와 권한 필터는 `lib/home/widgets.ts` + `availableWidgets(userId)` 가 담당한다.

| 항목 | 모바일 대응 |
|---|---|
| 스탯 4카드 | 같은 지표·델타 문구 규칙으로 이식(타일 4개, 탭하면 해당 화면) |
| 위젯 표시 여부 | **`/api/home/layout` 을 그대로 재사용** — available(권한)·hidden(웹에서 끈 카드)을 존중. 배치·크기 편집은 모바일에 두지 않고 **중요도 고정 순서** |
| 위젯 7종 | 수신 문서·발행 요청 수신함·발행 요청 대상·업무보고 할 일·경과 미입력·내 참여 용역·투찰 마감. 데이터가 없으면 카드가 스스로 사라진다 |
| 미이식 | 수금/발행 요약·인텔 하이라이트·임시 사업장 완성(데스크탑 성격), 캘린더(M6-B) |

**함정**: typedRoutes 가 엄격해져 `router.push(\`/board/${id}\`)` 같은 **템플릿 문자열을 더는 받지 않는다** → `{ pathname: '/board/[postId]', params: { postId } }` 형태로 교체.

### 10.8 M6-B 일정 캘린더 (2026-07-30, OTA 410b741c — 서버 변경 없음)
데스크탑 홈에 새로 생긴 월 캘린더의 모바일판. 데이터원 동일(영업 `/api/sales/schedule`, 휴가 `/api/home/calendar`).
- 월 격자 7열 + 오늘·선택일 표시. **일정 유무는 점으로만**(영업 파랑·휴가 초록) — 좁은 화면에서 셀 안에 제목을 넣으면 못 읽는다. 상세는 아래 리스트로 분리.
- 날짜 탭 → 그날 휴가·영업 활동 상세(연락처 tel/sms 유지). 휴가 토글은 끄면 요청도 보내지 않는다.

### 10.9 OTA 운영 — 진단 창구 신설 (2026-07-30)
