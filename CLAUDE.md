...
- **재무 통합 임시 배포 중지(2026-09-22):** 이 커밋의 Next는 새 재무 SQL 28개(번호 226~264, 다른 기능의 SQL 246·252~254·259·261·262는 제외)에 의존한다. 스테이징에 해당 28개 SQL을 번호순으로 한 번의 `-AccountingMaintenance` 절차로 모두 적용하고 성공을 확인하기 전에는 Next를 배포하지 않는다. SQL 적용이 실패하거나 복구 표식이 남으면 배포를 계속하지 않는다. 적용·배포 확인 뒤 이 임시 문구를 별도 커밋으로 제거한다.
