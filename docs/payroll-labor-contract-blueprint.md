...
> 후속(이번 범위 아님): 성과급 BS-P4(반기×직원 급여 그릇)
> ※ 급여명세서 개인 발송·4대보험/원천세 자동 계산은 P4에서 구현 완료(§6-2·§4-4).
...
### 4-4. 급여명세서 개인 발송 (2026-08-14 구현)
- 근거: 근로기준법 제48조② 임금명세서 교부 의무(구성항목·계산방법·공제내역 기재).
- 대상: **확정(confirmed) 대장**의 직원 매칭 행. 대장 화면 [명세서 발송] → 발송 모달(직원별 발행 이력·메일 등록 여부·미리보기, 미발행분 발송/전체 재발송).
- PDF(`lib/payroll/statement-pdf.ts`): 자간 제목 + 인적사항 2행(성명·소속·직급 / 사번·지급일·지급방법) + 지급/공제 4열 대조표 + 실지급액 박스 + **임금 계산 방법 박스**(초과근무수당 산출식 = 통상시급 × (연장×1.5 + 야간×2.0)·산정기간, 소득세=간이세액표, 공단 고지 반영 문구) + 회사명·대표이사·직인. 파일은 저장하지 않고 라인에서 온디맨드 렌더.
- 교부: 홈 위젯 **내 급여명세서**(최근 24개월 열람·내려받기, 본인 행만 403 가드) + 메일은 도착 알림만(임금 기밀, §8-7과 동일 기조). 발송 이력은 `payroll_entries.statement_sent_at`(마이그 164).
- 소속·직급은 매칭 직원이면 현재 프로필 값 우선(대장 스냅샷 공란 다수 — 2026-07 기준 31행 중 23행).
...
- 반영처:
  1. 근태·초과근무 관리 화면(`/approval/attendance`) 주별 그리드에 **예상 수당 금액 열** 추가 — **구현 완료(2026-08-14)**: 산정 로직은 `lib/payroll/overtime.ts` 공용 모듈(통상시급·배수·귀속구간)로 분리해 대장 생성과 동일 기준을 쓴다. 열에 통상시급 툴팁, KPI에 주간 예상 수당 합계, 계약 미등록자는 "계약 필요" 표기.
...
- 소득세 산정 파라미터(2026-08-14 확정): **원천징수 비율은 직원별 옵션**(80/100/120%, 기본 100 — 연말정산 시 개인이 선택하는 값이라 개인 설정으로 둠). **부양가족수는 세무사 소득세액공제신고서 엑셀(1-1시트: E4 성명·E11 비율 체크·C20~D31 관계코드/성명/주민번호/기본공제 O 명부) 파싱**으로 초기 적재 + 관리자 수기 보정. 2월 정산-건강보험 대량 환급 = **연말정산 환급액**(근로소득원천징수영수증 하단 차감징수세액) 반영분으로 확정.
- **간이세액표(161 `income_tax_brackets`) 시딩 완료(2026-08-14)**: 소스=공공데이터포털 국세청 파일(publicDataPk=15050747, hwpx — Contents/section0.xml tbl2), 2024.02.29 개정 현행본 646구간×11가족수=7,106행. 국세청 공식 예시(3,500천·4인=49,340) 검증 통과, 2026년 대장 소득세 79%가 100% 표값과 정확 일치(잔여는 비율 선택·자녀공제 차감분). 표 범위=월 770천~10,000천원(초과 구간은 산식 — 표 밖이면 수기 입력 유도). 자녀 세액공제: 8~20세 1명 12,500 / 2명 29,160 / 3명+ 29,160+초과당 25,000(`payroll_tax_profiles.child_deduction`).
- **보험요율 변동 자동 대응(2026-08-14 사용자 요청, 마이그 162 `payroll_insurance_rates`)**: 요율이 필요한 것은 자체 산정 2종뿐 — 건강보험·국민연금은 EDI 고지액 전기라 요율 인상이 자동 반영되고, 고용보험 근로자 부담분(실업급여)은 업종 무관 동일(업종별 차등은 산재보험 = 사업주 전액 부담이라 대장 공제 무관). ①**장기요양**: EDI 건강보험 파일 업로드 시 (요양÷건보 산출보험료) 중앙값을 역산해 해당 연도 요율 자동 upsert(source='edi-derived', 표본 5명 미만 보류, 수동 입력이 우선) ②**고용보험**: 법 개정이 드물어 설정 화면(세액 설정 탭 보험요율 카드)에서 수동 등록(적용 시작 YYYY-MM). 산식 모듈(tax.ts)은 하드코딩 대신 이 테이블을 조회.
- **간이세액표 자동 업데이트(2026-08-14 사용자 요청)**: 공공데이터포털 메타 API(`/tcs/dss/selectFileDataDownload.do?publicDataPk=15050747&publicDataDetailPk=<uddi>`)가 기준일(`stdrDe`)·첨부파일 ID(`atchFileId`)·차기 등록 예정일(`nextRegistPrarnde`)을 JSON으로 제공 — P4에서 급여 설정 화면에 **"간이세액표 버전 관리" 카드**(현재 버전·기준일 표시 + [개정 확인] 버튼: 메타 조회→기준일 변경 감지→hwpx 다운로드·파싱→새 year_version 시딩)를 구현. 개정 주기가 1~3년이라 상시 크론보다 **새 대장 생성 시 자동 체크 + 수동 버튼** 방식. 개정판 표 구조 변경으로 파싱 실패 시 관리자에게 수기 업로드 안내 폴백.
...
## 6-3. 자체 연말정산 (향후 계획 — P4 범위 아님, 2026-08-14 사용자 지시)

> 현재는 세무사 양식(소득세액공제신고서 엑셀)에 의존. 추후 자체 작성 전환을 대비해 **직원별 연말정산 자료 제출 메뉴**를 신설한다.
> ⚠ 지금 구현하지 않는다 — P4 완료 후 별도 단계(가칭 PL-P5)로 착수.

- **홈택스 연말정산 자료 PDF 업로드·분석**: 근로소득원천징수영수증 등 양식 생성 PDF는 텍스트 추출이 용이함을 실증(pypdf로 결정세액·차감징수세액 파싱 확인) — 환급/납부액 자동 인식 후 2월 대장 정산 항목 자동 반영.
- **피부양자 정보 입력**: 배우자·부모·자녀(관계코드·주민번호·기본공제 여부) — 간이세액표 공제대상가족수의 원천. 세무사 엑셀 파싱 초기값 + 직원 셀프 갱신.
- **소득세 원천징수 비율 선택**(80/100/120%) — §6-2 직원별 옵션과 동일 저장소.
- **의료비 지출 영수증·기부금 영수증 업로드 및 내역 입력** — 공제신고서 4·5-1시트 대응.
- 제출 현황 관리(연도별 제출/미제출, 관리자 리마인드).
