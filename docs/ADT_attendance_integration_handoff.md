...
- **집계 규칙**: 출근 = 당일 **첫 진입 태그**('1'·'5' 중 최초). 퇴근 = **출근 이후** 태그만
  후보로 '2' 최후, 없으면 '5' 최후(없으면 미정). 06:00 미만 새벽 태그는 전일에 태그가
  있으면 전일 퇴근으로 귀속(computeDaily 의 자정 +1일 보정과 연계 — 당일 태그끼리는
  out ≥ in 이 보장되므로 자정 보정은 이 경로에서만 발생).
  ⚠ 초기 규칙("출근 '1' 우선, 퇴근은 출근 이전 태그도 허용")은 아침 이중 태그
  (07:06 출입 → 07:20 출근 버튼)를 out=07:06/in=07:20 으로 잡아 자정 보정이 발동,
  가짜 철야 22h46m 을 만들었다(2026-08-31 실측 사고) — 위 규칙으로 교정.
  파서·집계 37케이스 검증 통과(사고 재현 H1~H3 포함).
...
4. **식대 위반 관리자 처분** (마이그 204) — 위반 1·2회는 경고, 반복 시 조치 원칙.
   근태 관리 화면 "식대 경고" 탭(`/api/payroll/meal-warnings`, admin)에서 건별 이력
   (사용일·사용처·금액·결제시각·신청/기준·누적 회차)을 보고 처분을 지정한다:
   경고(기본) / 불지급(기록·표시 — 개인영수증 환급 제외 등 지급 단계 처리) /
