// 기안 편집 진입 경로 — 양식 전용 작성 화면(공문·내부고시·견적서)과 범용 기안 화면을 구분한다.
...
export function approvalEditHref(formId: string | null | undefined, docId: string): string {
  ...
  if (formId === NOTICE_FORM_ID) return `/approval/notice${q}`;
...
/** 새 기안 진입 경로 — 전용 작성 화면이 있는 양식(공문·내부고시·견적서)은 그 화면으로 보낸다. */
export function approvalNewHref(formId: string): string {
  if (formId === LETTER_FORM_ID) return "/approval/letter";
  if (formId === NOTICE_FORM_ID) return "/approval/notice";
  if (formId === QUOTE_FORM_ID) return "/approval/quote";
  if (formId === AGREEMENT_FORM_ID) return "/contracts/agreements";
  return `/approval/draft?formId=${encodeURIComponent(formId)}`;
}
