...
- **인건비 반영 산정**(후속): 매출액에 N% 적용 후 **참여인력 반기 급여총액을 제한** 후 산정.
  25H2 엑셀 실물이 이 방식의 원형(성과산정액2=급여×1 차감, 성과산정액3=급여×1.5 차감).
  급여 데이터 구축 작업은 추후 별도 진행 예정 — 그때 반기×직원 급여 그릇(§3-6) 신설.
...
-- 6) (후속 — v1 제외) 반기 급여 입력, 인건비 반영 산정용. 급여 데이터 구축 시점에 별도 마이그레이션.
-- bonus_employee_salaries (period_year, period_half, employee_id, salary_total/base/overtime)
```
...
- **산정방식 옵션**(체크박스 3종, §1.3 — v1은 '기존 산정방식'만 활성, 인건비/영업이익은 disabled) +
  **비율 조정**(disabled 옵션 소속이므로 v1은 입력만 보이고 비활성). **admin 전용 표시**.
...
| **BS-P4** | (후속) 인건비/영업이익 산정방식 — 급여 데이터 구축 후 착수 | 후속 |
